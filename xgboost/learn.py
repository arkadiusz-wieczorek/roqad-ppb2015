...
dtrain = xgb.DMatrix(sys.argv[1] + 'DMatrixDataLearn.txt')
dtest = xgb.DMatrix(sys.argv[1] + 'DMatrixDataTest.txt')
...
dtest2 = xgb.DMatrix(sys.argv[2] + 'DMatrixDataCheck.txt')
ypred = bst.predict(dtest2)

with open(sys.argv[2] + 'prediction.txt', 'w') as predfile:   
    for value in ypred:
